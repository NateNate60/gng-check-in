...
export async function GET (request: NextRequest) {
    ...
    let [response, packets] = await connection.query("SELECT value FROM Settings WHERE setting = 'pokeball_days'")

    let cutoff = new Date()
    cutoff.setDate(cutoff.getDate() - Number(response[0]["value"]))

    let [response2, packets2] = await connection.query("SELECT pid FROM " +
